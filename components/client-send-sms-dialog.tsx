...
const PRESETS: Record<string, string> = {
  runningLate:
    "Hi {client_name}, we're running a few minutes late. We'll be there soon!",
  appointmentCancelled:
    "Hi {client_name}, we need to reschedule your appointment. Please give us a call to find a new time.",
  reschedule:
    "Hi {client_name}, we'd like to reschedule your appointment. Please call us to pick a new date and time.",
};
...
export function ClientSendSmsDialog({
  ...
  useEffect(() => {
    if (open) setMessageText(presetMessage ?? "");
  }, [open, presetMessage]);

  const handlePreset = (key: keyof typeof PRESETS) => {
    const text = PRESETS[key].replace(
      /\{client_name\}/g,
      clientName || "there",
    );
    setMessageText(text);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{title ?? t("sendSms")}</DialogTitle>
          <DialogDescription>
