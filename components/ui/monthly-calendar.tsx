...
export function MonthlyCalendar({
  ...
  return (
    <div
      className={cn(
        "w-full min-w-0 max-w-full rounded-xl border border-[#E2E8F0] dark:border-border bg-white dark:bg-card p-3 sm:p-5 md:p-6",
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0 flex-1">
          <div className="text-xs sm:text-sm font-medium text-muted-foreground">Bookings</div>
      ...
      <div className="mt-3 grid grid-cols-7 gap-0.5 sm:gap-1 md:gap-2">
        {DAY_NAMES.map((d) => (
          <div key={d} className="grid h-6 sm:h-8 place-items-center text-[10px] sm:text-xs font-medium text-muted-foreground truncate">
            {d}
          </div>
        ))}
        {cells.map(({ date, inMonth, key }) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative grid aspect-square w-full min-w-0 place-items-center rounded-md sm:rounded-lg text-xs sm:text-sm font-medium transition-all",
                "focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
                inMonth ? "text-foreground" : "text-muted-foreground/50",
                isSelected
                  ? "bg-primary text-primary-foreground shadow-[inset_0_1px_2px_rgba(0,0,0,0.1)]"
            ...
            >
              ...
              {hasBookings ? (
                <span
                  className={cn(
                    "absolute -right-0.5 -top-0.5 grid h-3.5 min-w-3.5 sm:h-4 sm:min-w-4 place-items-center rounded-full px-0.5 text-[9px] sm:text-[10px] font-semibold tabular-nums shadow-sm",
