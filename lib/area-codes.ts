...
export const AREA_CODES_BY_STATE_MAP = new Map<StateAbbrev, string[]>([
...
// Helper function to get all state abbreviations (sorted alphabetically)
export const getAllStates = (): StateAbbrev[] => {
  return Array.from(AREA_CODES_BY_STATE_MAP.keys()).sort();
};

// Helper function to get area codes for a state
export const getAreaCodesForState = (state: StateAbbrev | null): string[] => {
  if (!state) return [];
  return AREA_CODES_BY_STATE_MAP.get(state) || [];
};
